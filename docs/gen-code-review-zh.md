...
|----|------|
| **严重度** | **P1** |
| **现象** | ✅ Assembler、ApplicationService、QueryService 拆分、Repository 接口、Feign+Fallback、`GlobalExceptionHandler`（ProblemDetail）均有。❌ 无 skill 的 **ReadMapper + XML 投影**（读侧仍走领域对象再 Assembler）；❌ `PageResult` 放在 **contract** 而非 **common-core**；❌ 无统一 `BusinessException` / 多异常映射；✅ 有意不使用 `R`/`ApiResponse` 包装（与 skill「标准 REST 直接返回 DTO/PageResult」一致）。 |
...
|----|------|
| **严重度** | **P2** |
| **现象** | Controller 有 `@Tag`/`@Operation`/`@Parameter`；DTO 有 `@Schema`。缺：分页查询参数未强制 `@Validated`；部分响应字段 schema 深度一般；Feign 方法 swagger 视开关而定。 |
...
|----|------|
| **严重度** | **P1**（ID/雪花）；**P2**（其余） |
| **现象** | Create 用 `@NotBlank`/`@Size`；ID 为 **去掉横线的 UUID 字符串**（`UUID.randomUUID()`），非 Snowflake / `IdType.ASSIGN_ID`；PO 有 `@TableLogic` + `deleted INT`；契约有 `StatusEnum`，领域另有 `XxxStatus`——双轨可接受但描述字段弱于 skill（skill 枚举带 description）。软删类型 skill 用 `BOOLEAN`，我们用 `INT 0/1`。 |
...
|----|------|
| **严重度** | **P2** |
| **现象** | `ImportExportService` 返回 `Map.of("accepted", true, "message", "…stub…")`，可编译但无 EasyExcel、无流式响应、无文件校验。 |
...
未纳入本轮（建议下一迭代）：Snowflake、ReadMapper CQRS、Query `@Validated`、EasyExcel 真实现、Flyway COMMENT。
